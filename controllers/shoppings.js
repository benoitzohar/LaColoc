...
var getCurrentShoppingListData = function(group) {
    ...
    q.all(
            Shopping.current(group._id),
            Shopping.archiveList(group._id)
        )
        .then(function(data) {
                ...
                if (!data[0]) {
                    ...
                    new_shopping.save(function(err, new_shopping_saved) {
                        d.resolve({
                            entity_id: new_shopping_saved._id,
                            items: new_shopping_saved.items,
                            archives: data[1]
                        });
                    });
                } else {
    ...
    if (req.shopping && req.body.items) {
        req.shopping.updateItems(req.body.items, true)
            .then(function(saved) {
                    ...
                    // broadcast to rest of group
                    socket.broadcastToGroup(req.user, 'shopping:list', {
                        items: saved.items
                    });

                    //send answer to current user
                    utils.sendJSON(res, null, {
                        items: saved.items
                    });
