...
class App {
	...
	static private $modules_tablename = 'lc_modules';
	static private $modules_users_tablename = 'lc_modules_users';
	...
	public function getLogo() { 
		if (!empty($this->logo))
			return $this->logo;
		else if (is_file(LC::$path.'modules/'.$this->name.'/img/logo.png')) {
			return LC::$url.'modules/'.$this->name.'/img/logo.png';
		}
	 } 
	public function getPageSetup() {
		if (!$this->page_setup) return '';
		if (is_file(LC::$path.'modules/'.$this->name.'/'.$this->page_setup)) {
			return LC::$path.'modules/'.$this->name.'/'.$this->page_setup;
		}
	...
	public function getPageAdmin() {
		...
		if (is_file(LC::$path.'modules/'.$this->name.'/'.$this->page_admin)) {
			return LC::$path.'modules/'.$this->name.'/'.$this->page_admin;
		}
	...
	public function getPageMain() {
		...
		if (is_file(lC::$path.'modules/'.$this->name.'/'.$this->page_main)) {
			return LC::$path.'modules/'.$this->name.'/'.$this->page_main;
		}
	...
	static public function get_all($user = false,$order = false, $only_names = false) {
    	...
    	if ($sql)
	    	while (!$sql->EOF){
		    	...
		    	else array_push($res,new App($sql->fields['name'],$sql->fields['title'],$sql->fields['version'],$sql->fields['last_edit'],$sql->fields['logo'],$sql->fields['page_setup'],$sql->fields['page_admin'],$sql->fields['page_main'],$sql->fields['tables']));
	    		$sql->MoveNext();
	    	}
    	return $res;
