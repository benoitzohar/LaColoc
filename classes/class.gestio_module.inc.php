...
class gestio_module {
	...
	public function has_module_structure() {
		if ($this->has_module_structure)
			return true;
    	$module_path = Gestio::$path.'modules/';
	...
	public function import_module_configuration() {
		...
		if (!empty($gestio_module[$this->name]) && is_array($gestio_module[$this->name])) {
			$this->properties = $gestio_module[$this->name];
		}
	}
	...
	public function is_installed() {
		...
		$app_installed  = new gestio_app($this->name);
	...
	public function install($current_version = 1.0) {
		for($v = $current_version;$v<=$this->get_version();$v = $v+0.1) {
			$fic = Gestio::$path.'modules/'.$this->name.'/setup/tables-'.number_format($v,1).'.sql';
		...
		$new_app = new gestio_app(	$this->name,
	...
	public function update() {
		$app_to_update = new gestio_app($this->name);
		return $this->install($app_to_update->getVersion());
	}
	
	public function uninstall() {
		$app_to_uninstall = new gestio_app($this->name);
